...
/* ---------------- أنواع الأرز ----------------
   تُختار مع اللقن بلا فرق سعر، ويُسمح بنوعين على الأكثر. واختياريّة:
   لا توقف الطلب من لا رأي له في الأرز، وللمطبخ ما اعتاده.
...
/* لا اختيار أرز في اللحم والدجاج: اسم الطبق يحدّده أصلاً — «دجاج على
   كبسة برية» و«خروف على برياني عربي» — فكان الاختيار يفتح باباً لطلب
   يخالف اسم الطبق نفسه. من أراد أرزاً غيره كتبه في ملاحظات المطبخ.
   ويبقى الاختيار في السمك وحده (RICE_PICKS) اختيارياً، فاللقن لا
   يحمل في اسمه نوع أرزه. */

/* «سلطة بيت» لا «سلطة» وحدها: السلطات المسمّاة في الإضافات مدفوعة،
   فلو قيل «سلطة مجاناً» ظنّ الزبون أنه يُحاسَب على المجاني */
var MEAT_INCLUDED = 'مع كل طلب: سلطة بيت + مرق مجاناً';
...
var MENU = [
  ...
  lambFull('lamb-biryani-arabi', 'خروف على برياني عربي',  2000, 3000),
...
var ORDER_CONFIG = {
  ...
  /* ---------- تحديد الموقع ----------
     الزبون يضغط «حدّد موقعي» فتُقاس المسافة بين إحداثيّاته ومركز
     الدوحة، وتُضبط منطقته على ما توافقها. الغرض ألّا يصل طلبٌ من
     خارج الدوحة برسم داخلها — لا منعاً للزبون بل معرفةً للمطبخ:
     الرسالة تحمل رابط موقعه، وتقول صراحةً إن خالف اختيارُه موقعَه.

     insideKm نصف قطر «داخل الدوحة» بالكيلومترات. ٢٥ تغطّي الدوحة
     الكبرى — الوكرة وأم صلال والريان — وتُخرج الخور والشمال ومسيعيد.
     زِدها أو أنقصها بحسب ما تعدّه داخلاً. */
  geo: {
